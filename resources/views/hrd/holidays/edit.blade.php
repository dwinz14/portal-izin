<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Perbarui data tanggal merah atau cuti bersama.
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div
            ...
            <form method="POST" action="{{ route('hrd.holidays.update', $holiday->id) }}" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="type" name="type"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm">
                        <option value="national_holiday" @selected(old('type', $holiday->type) === 'national_holiday')>
                            Libur Nasional (tanggal merah — tidak potong kuota)
                        </option>
                        <option value="joint_leave" @selected(old('type', $holiday->type) === 'joint_leave')>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="2"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm resize-none">{{ old('description', $holiday->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center justify-end space-x-3 pt-2">
                    <a href="{{ route('hrd.holidays.index', ['year' => $holiday->year]) }}"
