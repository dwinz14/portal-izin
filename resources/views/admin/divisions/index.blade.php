<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div
                ...
                <a href="{{ route('admin.divisions.sync') }}"
    ...
    <div class="space-y-4" x-data="{ showFilters: false, confirmDelete: false, selectedDivision: null, selectedDivisionName: '' }">
        @if (session('success'))
            <div
            ...
            <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-emerald-600 dark:text-emerald-400">
                                {{ $newDivisi->count() }} divisi baru ditemukan dari database karyawan.
                            </p>
                        </div>
                    </div>
                    @if ($newDivisi->isNotEmpty())
                        <form action="{{ route('admin.divisions.insertAllFromExternal') }}" method="POST">
                            @csrf
                            <button type="submit"
                ...
                @if ($newDivisi->isEmpty())
                    <div class="px-5 py-8 text-center">
                        ...
                        <p class="mt-2 text-sm font-medium text-gray-600 dark:text-gray-400">Semua divisi sudah sinkron.</p>
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Tidak ada divisi baru dari database
                            karyawan.</p>
                    </div>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            <thead class="bg-emerald-50 dark:bg-emerald-950/20">
                                <tr>
                                    ...
                                    <th
                                        class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Nama Divisi (dari DB Karyawan)</th>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Aksi</th>
                                </tr>
                            ...
                            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-slate-700/60">
                                @foreach ($newDivisi as $index => $nama)
                                    <tr class="hover:bg-emerald-50/40 dark:hover:bg-emerald-950/10 transition-colors">
                                        ...
                                        <td class="px-4 py-3 text-sm">
                                            <form action="{{ route('admin.divisions.insertFromExternal') }}"
                                                method="POST">
                                                @csrf
                                                <input type="hidden" name="nama_divisi" value="{{ $nama }}">
                                                <button type="submit"
                        ...
                        @forelse ($divisions as $index => $division)
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 font-medium">
                                    {{ strtoupper($division->nama_divisi) }}</td>
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button
                                            @click="confirmDelete = true; selectedDivision = {{ $division->id }}; selectedDivisionName = '{{ $division->nama_divisi }}'"
                        ...
                        @empty
                            <tr>
                                <td colspan="3"
                                    ...
                                    <p class="mt-2">Tidak ada data divisi ditemukan.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="confirmDelete" x-transition x-cloak style="display: none;"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md transform transition-all duration-300 ease-out"
                ...
                <h2 class="text-lg font-semibold mb-4">Hapus Divisi</h2>
                <p class="mb-4">
                    Apakah Anda yakin ingin menghapus divisi <span class="font-bold"
                        x-text="selectedDivisionName"></span>?
                </p>
                <form x-bind:action="'{{ route('admin.divisions.destroy', ':id') }}'.replace(':id', selectedDivision)"
                    method="POST">
                    @csrf
                    @method('DELETE')
